      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4">
        <h4
          className="text-3xl hidden sm:inline sm:text-4xl
            md:text-5xl font-semi-bold text-slate-400"
        >
          0{i + 1}
        </h4>
        <h2
          className="capitalize whitespace-nowrap truncate
        max-w-full text-lg sm:text-xl md:text-2xl
        flex-1 sm:text-center"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        {["reps", "rest", "tempo"].map((info) => {
          return (
            <div
              key={info}
              className="flex flex-col rounded border-[1.5px] 
                p-2 border-solid border-slate-900 w-full"
            >
